...
List<DayLog> cyclesOf(DateTime start, List<int> lengths, {int periodDays = 4}) {
  final logs = <DayLog>[];
  var cursor = start;
...
void main() {
  group('insufficient data', () {
    ...
    test('one completed cycle is still not enough', () {
      final cycles = analyzer.analyze(
        cyclesOf(DateTime(2026, 1, 1), const [28]),
      );
      final p = engine.predict(cycles);
  ...
  group('regular cycles', () {
    test('predicts a window centred on the mean', () {
      final logs = cyclesOf(DateTime(2026, 1, 1), const [
        28,
        28,
        28,
        28,
    ...
    test(
      'perfectly regular history still yields a range, never a single day',
      () {
        final logs = cyclesOf(DateTime(2026, 1, 1), const [
          28,
          28,
  ...
  group('irregular cycles', () {
    test('wider variability produces a wider window and lower confidence', () {
      final regular = engine.predict(
        analyzer.analyze(
          cyclesOf(DateTime(2026, 1, 1), const [28, 28, 28, 28]),
        ),
      );
      final erratic = engine.predict(
        analyzer.analyze(
          cyclesOf(DateTime(2026, 1, 1), const [22, 35, 26, 40]),
        ),
      );
  ...
  group('robustness', () {
    test('an implausibly long gap is excluded from the statistics', () {
      ...
      final logs = cyclesOf(DateTime(2024, 1, 1), const [
        28,
        28,
        400,
        28,
