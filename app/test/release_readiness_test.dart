...
void main() {
  ...
  group('the Premium dev cheat stays out of release builds', () {
    ...
    test('the toggle is behind kDebugMode', () {
      ...
      expect(source, contains('premiumDevToggle'),
      ...
      expect(source.indexOf('kDebugMode'),
    ...
    test('no other screen exposes a premium override', () {
      ...
      expect(offenders, isEmpty,
          reason: 'setPremiumActive called from an unreviewed place: '
              '$offenders');
    });
  });
  ...
  group('AdMob ids', () {
    ...
    test('the test-id flag matches what the ids actually are', () {
      ...
      expect(hasSampleIds, claimsTestIds,
          reason: hasSampleIds
    ...
    test('the app id in each platform manifest agrees with ad_ids.dart', () {
      ...
      final usingTest =
          read('lib/services/ad_ids.dart').contains(googleSamplePublisher);
      final android =
          read('android/app/src/main/AndroidManifest.xml').contains(
        googleSamplePublisher,
      );
      final ios = read('ios/Runner/Info.plist').contains(googleSamplePublisher);

      expect(android, usingTest,
          reason: 'AndroidManifest APPLICATION_ID disagrees with ad_ids.dart');
      expect(ios, usingTest,
          reason: 'Info.plist GADApplicationIdentifier disagrees with '
              'ad_ids.dart');
    });
  });

  group('Android release signing', () {
    test('release is not signed with the debug key', () {
      ...
      expect(debugSignedRelease, isFalse,
          reason: 'release builds still use the debug signing config — '
              'create a keystore and a signingConfigs.release block');
    }, skip: 'Known gap: no keystore exists yet. Un-skip when one is '
